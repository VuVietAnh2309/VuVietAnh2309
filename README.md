<!-- Banner: auto dark/light -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/banner-dark.svg">
  <img alt="Viet Anh Vu — AI Engineer" src="assets/banner-light.svg">
</picture>

<!-- Typing animation -->
<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=3000&pause=1000&color=A855F7&center=true&vCenter=true&random=false&width=620&lines=Healthcare+AI+%7C+EdTech+AI+%7C+MLOps;LLMs+%26+RAG+%7C+Computer+Vision+%7C+NLP;Building+AI+systems+that+matter+%F0%9F%9A%80" alt="Typing SVG"/>
</p>

<!-- Badges -->
<p align="center">
  <a href="mailto:mrvietanh2@gmail.com">
    <img src="https://img.shields.io/badge/Email-mrvietanh2%40gmail.com-informational?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/>
  </a>
  <a href="https://www.linkedin.com/in/viet-anh-vu-30a268336/">
    <img src="https://img.shields.io/badge/LinkedIn-Viet%20Anh%20Vu-blue?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
  </a>
  <a href="data/resume.pdf">
    <img src="https://img.shields.io/badge/CV-resume.pdf-critical?style=for-the-badge&logo=adobeacrobatreader&logoColor=white" alt="CV"/>
  </a>
</p>
<p align="center">
  <img src="https://img.shields.io/badge/Hanoi-Viet%20Nam-orange?style=flat-square&logo=google-maps&logoColor=white" alt="Location"/>
  <img src="https://komarev.com/ghpvc/?username=VuVietAnh2309&label=Profile%20views&style=flat-square" alt="Visitors"/>
</p>

---

## 🧑‍💻 About me

> MSc student & AI Engineer with 2+ years building production ML/LLM systems in Healthcare and Education.

- 🎓 **MSc, Computer Science** @ HUST (Jun 2025 – Present) | BSc (Honours), GPA 3.48/4.0
- 🏢 AI Engineer @ **Prep Education** — Automated Essay Scoring & Grammar Correction for IELTS
- 🏥 Previously @ **Vietsens Technology Group** — Healthcare AI for Bach Mai Hospital
- 📄 Published at **IEEE SCCI 2024** | 2 manuscripts under review
- 🏆 **WiDS Datathon 2023** — 2nd Prize

---

## 💼 Experience

### <img src="https://img.shields.io/badge/Current-00c853?style=flat-square" alt="Current"/> Prep Education — AI Engineer (Oct 2025 – Present)

<details>
<summary>🎯 <b>Automated Essay Scoring (AES)</b> — IELTS Writing & Speaking</summary>
<br/>

| Component | Details |
|-----------|---------|
| **Speaking** | ML + APA (pronunciation) + GEC (grammar) + Dify (vocabulary) |
| **Writing** | Cross-prompt DeBERTa-V3 (189M), quantized T5 (120M) |
| **Metrics** | Speaking: Acc ±0.5 ~82%, QWK & PCC ~90% · Writing: Acc ±0.5 ~85–87%, ±1 ~98% |
| **MLOps** | Docker → GitLab CI/CD → ArgoCD → Kubernetes |
| **Scale** | Dynamic batching & request queuing for thousands of concurrent users |
</details>

<details>
<summary>✏️ <b>English Lexical Grammar (ELG) Correction</b> — Rule-based + LLMs</summary>
<br/>

- Hybrid grammar correction: rule-based pattern matching + LLM inference
- Covers: grammar, vocabulary, spelling, collocations, idiomatic expressions, linking devices
- **F0.5 ~60%** on private test sets
- Production: Docker + GitLab CI/CD + ArgoCD on K8s, request batching & caching
</details>

### <img src="https://img.shields.io/badge/2024--2025-1e88e5?style=flat-square" alt="2024-2025"/> Vietsens Technology Group — AI Engineer

<details>
<summary>🏥 <b>AI Healthcare Assistant</b> — LLMs, NLP, HIS integration</summary>
<br/>

- AI agents for medical history inquiry & initial diagnosis
- Integrated with **National Hospital Management Software** (Bach Mai Hospital)
- Modules for patient data analysis, case management, and diagnostic support
</details>

<details>
<summary>💊 <b>Prescription Recommendation & Conflict Detection</b></summary>
<br/>

- Predictive system for medication prescribing based on patient records
- LLMs for duplicate active ingredient detection & drug-drug interaction identification
</details>

### <img src="https://img.shields.io/badge/Intern-ff9800?style=flat-square" alt="Intern"/> FPT Information System — AI Engineer Intern (Aug 2023 – May 2024)

<details>
<summary>📄 <b>OCR for Financial Reports</b> — PaddleOCR, TrOCR, LayoutLMv3</summary>
<br/>

- End-to-end OCR pipeline: preprocessing → text detection → layout understanding → NER + regex normalization
- Table & key-value extraction from bank financial reports
</details>

---

## 📚 Research & Publication

<table>
  <tr>
    <td>📖</td>
    <td>
      <b>VISTA</b> — Variable-Length GA + LSTM Surrogate for Ensemble Selection in MLES<br/>
      <sub>Kate Han, Truong Thanh Nguyen, <b>Viet Anh Vu</b>, Alan W.-C. Liew & Tien Thanh Nguyen</sub><br/>
      <img src="https://img.shields.io/badge/IEEE_SCCI_2024-Published-success?style=flat-square" alt="Published"/>
    </td>
  </tr>
  <tr>
    <td>🔬</td>
    <td>
      <b>Application of Deep Learning Network in Underwater Image Recognition and Segmentation</b><br/>
      <sub>ResNet denoising + UNet_v2 / V_UNet</sub><br/>
      <img src="https://img.shields.io/badge/Status-Under_Review-yellow?style=flat-square" alt="Under Review"/>
    </td>
  </tr>
  <tr>
    <td>🔬</td>
    <td>
      <b>Enhancing Facial Expression Recognition with Lightweight Attention-Based CNN</b><br/>
      <sub>MobileNetV2 + Patch Extraction + Attention for edge devices</sub><br/>
      <img src="https://img.shields.io/badge/Status-Under_Review-yellow?style=flat-square" alt="Under Review"/>
    </td>
  </tr>
</table>

---

## 🛠️ Skills & Tools

<table>
  <tr>
    <td><b>Languages</b></td>
    <td>
      <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white"/>
      <img src="https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white"/>
      <img src="https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnubash&logoColor=white"/>
      <img src="https://img.shields.io/badge/SQL-336791?style=flat-square&logo=postgresql&logoColor=white"/>
    </td>
  </tr>
  <tr>
    <td><b>ML / DL</b></td>
    <td>
      <img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white"/>
      <img src="https://img.shields.io/badge/Keras-D00000?style=flat-square&logo=keras&logoColor=white"/>
      <img src="https://img.shields.io/badge/OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white"/>
      <img src="https://img.shields.io/badge/HuggingFace-FFD21E?style=flat-square&logo=huggingface&logoColor=black"/>
    </td>
  </tr>
  <tr>
    <td><b>MLOps</b></td>
    <td>
      <img src="https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white"/>
      <img src="https://img.shields.io/badge/Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white"/>
      <img src="https://img.shields.io/badge/GitLab_CI/CD-FC6D26?style=flat-square&logo=gitlab&logoColor=white"/>
      <img src="https://img.shields.io/badge/ArgoCD-EF7B4D?style=flat-square&logo=argo&logoColor=white"/>
    </td>
  </tr>
  <tr>
    <td><b>Focus</b></td>
    <td>LLMs · RAG · Computer Vision · NLP · Ensemble Learning · Genetic Algorithms</td>
  </tr>
  <tr>
    <td><b>OCR</b></td>
    <td>PaddleOCR · Tesseract · TrOCR · LayoutLM/v3</td>
  </tr>
</table>

---

## 🎓 Education

| | Degree | Institution | Period |
|---|--------|------------|--------|
| 🎓 | **MSc, Computer Science** | HUST | Jun 2025 – Present |
| 🎓 | **BSc (Honours), Computer Science** | HUST — GPA 3.48/4.0 | Oct 2020 – Jul 2024 |

---

## 📈 GitHub at a glance

<p align="center">
  <img src="https://streak-stats.demolab.com?user=VuVietAnh2309&theme=radical&hide_border=true&border_radius=10" width="55%" alt="Streak Stats"/>
</p>

<table>
  <tr>
    <td width="50%">
      <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=VuVietAnh2309&theme=radical" width="100%"/>
    </td>
    <td width="50%">
      <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=VuVietAnh2309&theme=radical" width="100%"/>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=VuVietAnh2309&theme=radical" width="100%"/>
    </td>
    <td width="50%">
      <img src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=VuVietAnh2309&theme=radical" width="100%"/>
    </td>
  </tr>
</table>

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:00d2ff,50:a855f7,100:00f5a0&height=80&section=footer"/>
</p>
